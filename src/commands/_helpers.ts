...
export function resolveVault(): { vaultDir: string } {
...
export async function requirePassword(vaultDir: string): Promise<string> {
  ...
  const pw = await readPasswordWithRetry(
    "主密码: ",
    async (pw) => {
      try {
